...
class TICC:
    ...
    def fit(self, input_file,sc=None):
        """
        ...
        # print("The gmm clusters")
        # for c in gmm_clustered_pts:
        #     print(c)
        # K-means
        ...
        for iters in range(self.maxIters):
            ...
            opt_res = list(map(lambda x: x.get(), opt_res))
            opt={}
            for idx in range(len(opt_res)):
                opt[idx]=opt_res[idx]
            rdd1 = sc.parallelize(list(opt.items()))
            rdd2 = rdd1.map(lambda x: (dict([x])))
            computed_covariance=rdd2.map(lambda x: self.optimize_clusters(len_train_clusters, log_det_values,x,
                                 train_cluster_inverse)).collect()
            ...
            lle = rdd.map(self.predict_clusters)
            lle = lle.collect()
            lleDup = []
            for pt in lle:
                lleDup.append(pt[0])
            lle = np.asarray(lleDup)
            clustered_points = updateClusters(lle, switch_penalty=self.switch_penalty)
        
            # print("&&&&&&&&&&&&&&&&&&&&&&&")
            # print("&&&&&&&&&&&&&&&&&&&&&&&")
            # print("&&&&&&&&&&&&&&&&&&&&&&&")
            # print("&&&&&&&&&&&&&&&&&&&&&&&")
            # print(type(clustered_points))
            # print(len(clustered_points))
            # print(clustered_points)
            # print(clustered_points)
            # recalculate lengths
            
            # clustered_points = clustered_pointsDup
            new_train_clusters = collections.defaultdict(list) # {cluster: [point indices]}x
            ...
            if iters != 0:
                cluster_norms = [(np.linalg.norm(old_computed_covariance[i][self.number_of_clusters, i]), i) for i in
    ...
    def smoothen_clusters(self, cluster_mean_info, computed_covariance,
        ...
        for cluster in range(self.number_of_clusters):
            cov_matrix = computed_covariance[cluster][self.number_of_clusters, cluster][0:(self.num_blocks - 1) * n,
        ...
        for point in range(clustered_points_len):
            if point + self.window_size - 1 < complete_D_train.shape[0] :
                for cluster in range(self.number_of_clusters):
                    ...
                    #print(complete_D_train)
                    # tmp = complete_D_train[point, :]
                    tmp1 = cluster_mean_stacked[0:(self.num_blocks - 1) * n]
                    x = complete_D_train - cluster_mean_stacked[0:(self.num_blocks - 1) * n]
    ...
    def optimize_clusters(self, len_train_clusters, log_det_values, optRes, train_cluster_inverse):
        #for cluster in range(self.number_of_clusters):
        ...
        cc[self.number_of_clusters, cluster] = cov_out
        train_cluster_inverse[cluster] = X2
        #for cluster in range(self.number_of_clusters):
            #clprint("length of the cluster ", cluster, "------>", len_train_clusters[cluster])
...
# Reponsible for converting data points to windows n is number for cols
